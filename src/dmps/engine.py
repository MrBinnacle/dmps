...
class OptimizationEngine:
    ...
    def _determine_expected_output_format(self, user_prompt: str) -> str:
        ...
        analyzed_prompt = user_prompt[:self.MAX_REGEX_INPUT] if len(user_prompt) > self.MAX_REGEX_INPUT else user_prompt
    ...
    def _extract_user_constraints(self, user_prompt: str) -> List[str]:
        ...
        safe_prompt = user_prompt[:self.MAX_REGEX_INPUT] if len(user_prompt) > self.MAX_REGEX_INPUT else user_prompt
    ...
    def _identify_missing_info(self, prompt: str, intent: str) -> List[str]:
        ...
        try:
            if intent == "technical" and not re.search(r"\b(?:context|background|use case)\b", 
                                                      prompt, re.IGNORECASE):
                missing.append("Technical context might be helpful")
            
            if intent == "creative" and not re.search(r"\b(?:style|tone|audience)\b", 
                                                     prompt, re.IGNORECASE):
